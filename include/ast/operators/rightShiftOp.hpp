#ifndef rightShiftOp_hpp
#define rightShiftOp_hpp

class rightShiftOp;
typedef const rightShiftOp* rightShiftOpPtr;

class rightShiftOp: public Expression {
public:
    rightShiftOp(ExpressionPtr left, ExpressionPtr right): exp1(left), exp2(right) { branches.push_back(left); branches.push_back(right); }
    void mipsGen(std::ostream& os, mipsCon& con, int dest=0) const { 
        ...
        os << std::endl;
        os << "srl "<< con.reg(dest) << ", " << con.reg(addrDest1) << ", " << con.reg(addrDest2);
        os << std::endl;
